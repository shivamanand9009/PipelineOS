...
const styles = {
  ...
  handleWrapper: (side) => ({
    ...
    alignItems: 'center',
    gap: '5px',
    ...(side === 'left' ? { left: '-8px', flexDirection: 'row' } : { right: '-8px', flexDirection: 'row-reverse' }),
  }),
  handleLabel: (side) => ({
    fontSize: '9px',
    color: '#3a5a8a',
...
export const BaseNode = ({
  ...
  const nodeHeight = 80 + fields.length * 52 + (children ? 40 : 0);
  ...
  return (
    <div style={{ ...styles.node(accentColor), minWidth: minWidth || 220 }}>
      ...
      {inputHandles.map((h, i) => {
        const top = inputHandles.length === 1
          ? '50%'
      ...
      {outputHandles.map((h, i) => {
        const top = outputHandles.length === 1
          ? '50%'
